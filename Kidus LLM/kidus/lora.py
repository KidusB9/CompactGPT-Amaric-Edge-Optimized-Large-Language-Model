...
class LoRALinear(nn.Linear, LoRALayer):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.r > 0. and not self.merged:
            ...
            ab = (self.lora_dropout(x) @ self.lora_A @ self.lora_B.T)*self.scailing 
            result = result + ab
        else:
            result = F.linear(x, self.weight, bias=self.bias)
...
class CasualAttention(Kidus.CasualAttention):
    def __init__(self, config: Kidus.KidusConfig) -> None:
        nn.Module.__init__(self)

        assert config.n_embd % config.n_heads == 0, "Pleale verfiy  Embedding and Heads Number Configration."
...
def lora(r, alpha, dropout):
    ...
    causal_self_attention = Kidus.CasualAttention
    Kidus.CausalSelfAttention = CasualAttention
    yield
    Kidus.CausalSelfAttention = causal_self_attention
