...
class Kidus(nn.Module):
    def __init__(self, config: KidusConfig, pretrain:bool=True) -> None:
        super().__init__()
        self.config = config
    ...
    def forward(self, idx,  max_seq_length=None, input_pos=None):
        ...
        assert T <= max_seq_length, f"Cannot forward sequence of length {T}, max seq length is  {max_seq_length}"
        assert max_seq_length <= block_size, f"Cannot attend to {max_seq_length}, block size is  {block_size}"
    ...
    def from_name(cls, name: str, pretrain:bool=False) -> Self:
        config = KidusConfig.from_name(name)
    ...
    def reset_cache(self) -> None:
        ...
        if self.mask_cache.device.type == "xla":
            self.rope_cache = self.rope_cache.detach()
            self.rope_cache = None
            self.mask_cache = None
    ...
    def configure_optimizers(self, weight_decay=1e-1):
        ...
        assert len(inter_params) == 0, "parameters %s Proceding to no_decay sets!" % (str(inter_params), )
...
class Block(nn.Module):
    def __init__(self, config: KidusConfig):
        super().__init__()
...
class CasualAttention(nn.Module):
    def __init__(self, config: KidusConfig) -> None:
        super().__init__()
        assert config.n_embd % config.n_heads == 0, "Please Check Embedding settings and Heads Number Configrations."
...
def build_rope_cache(
    ...
    # Create $\theta_i$ for $i \in [1, 2, ..., \frac{d}{2}]$
...
if __name__ == "__main__":
    Kidus = Kidus.from_name("base", pretrain=True)
    print(Kidus)
    # Kidus.configure_optimizers()
